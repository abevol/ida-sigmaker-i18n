...
if not SIMD_SPEEDUP_AVAILABLE:
    ...
    if not SIMD_SPEEDUP_AVAILABLE:
        import importlib.machinery
        for p in sys.path:
            d = pathlib.Path(p) / "sigmaker" / "_speedups"
            if d.is_dir():
                files = [str(f.name) for f in d.iterdir()]
                print(f"[sigmaker] Found _speedups: {d}")
                print(f"[sigmaker]   Files: {files}")
                for suffix in importlib.machinery.EXTENSION_SUFFIXES:
                    if (d / f"simd_scan{suffix}").exists():
                        print(f"[sigmaker]   HAS simd_scan{suffix}")
                    else:
                        print(f"[sigmaker]   NO simd_scan{suffix}")
        print("[sigmaker] SIMD speedup NOT enabled")
    ...
    def find_xrefs(self, ea: int, cfg: SigMakerConfig) -> XrefGeneratedSignature:
        ...
        for i, frm_ea in enumerate(self.iter_code_xrefs_to(ea), start=1):
            ...
            self.progress_dialog.replace_message(_("progress.xref_progress") % {
                ...
                "total": total,
                "pct": (i / total) * 100.0,
                "suitable": len(xref_signatures),
                "shortest": shortest_len if shortest_len <= cfg.max_xref_signature_length else 0,
            })
...
class _FunctionSigProgress:
    ...
    def __call__(self, idx, item, elapsed, total) -> str:
        ...
        anchor_ea = item if isinstance(item, int) else self.current_anchor_ea
